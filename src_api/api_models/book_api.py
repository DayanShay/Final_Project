...
from src_api.obj_models.book import Book
from src_api.obj_models.book_dto import BookDto
...
class Books_Api(BaseApi):
    ...
    @BaseApi.make_a_req(url="api_models/Books", action="get")
    ...
    @BaseApi.make_a_req(url="api_models/Books", action="post")
    def post_books(self, response):
        if response.ok:
            return Book(**response.json())
        return API_Func.bad_respone_msg(response.status_code, response.text)

    @BaseApi.make_a_req(url="api_models/Books/", action="get", param="id")
    def get_books_by_id(self, response):
        if response.ok:
            return BookDto(**response.json())
        return API_Func.bad_respone_msg(response.status_code, response.text)

    @BaseApi.make_a_req(url="api_models/Books/", action="put", param="id")
    def put_books_by_id(self, response):
        if response.ok:
            return response.reason
        return API_Func.bad_respone_msg(response.status_code, response.text)

    @BaseApi.make_a_req(url="api_models/Books/", action="delete", param="id")
    def delete_books_by_id(self, response):
        if response.ok:
            return response.reason
        return API_Func.bad_respone_msg(response.status_code, response.text)

    @BaseApi.make_a_req(url="api_models/Books/findauthor/", action="get", param="authorid")
    ...
    @BaseApi.make_a_req(url="api_models/Books/purchase/", action="put", param="id")
